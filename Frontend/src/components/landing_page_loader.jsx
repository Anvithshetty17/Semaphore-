...
const CyberpunkLoader = ({ onLoadComplete }) => {
  ...
  // Random page glitch effect
  useEffect(() => {
    const glitchInterval = setInterval(() => {
      // Random chance to trigger glitch (5% chance every 200ms)
      if (Math.random() < 0.05) {
        setIsGlitching(true);
        // Glitch duration between 50-200ms
  ...
  useEffect(() => {
    // Only generate hexCodes on client to avoid hydration mismatch
    setHexCodes(Array.from({ length: 50 }, () => Math.random().toString(16).substr(2, 6).toUpperCase()));
  }, []);

  // Image paths
  const bgImg = isMobile ? "/images/loading_bg_mobile.png" : "/images/loading_bg_pc.png";
  const logoImg = "/images/semaphore_logo.png";
  const textImg = "/images/semaphore_text.png";
    ...
    <AnimatePresence>
      {(!isComplete || progress < 100) && (
        <motion.div
          ...
          className={`fixed inset-0 z-50 overflow-hidden ${isGlitching ? "animate-pulse" : ""}`}
          style={{
            filter: isGlitching
              ? `hue-rotate(${Math.random() * 360}deg) saturate(${1 + Math.random() * 2}) contrast(${
                  1 + Math.random()
                })`
              : "none",
            transform: isGlitching
              ? `translate(${(Math.random() - 0.5) * 10}px, ${(Math.random() - 0.5) * 10}px) scale(${
          ...
          <div className="relative z-10 h-full flex flex-col items-center justify-center px-8">
            ...
            <motion.div
              ...
              <p className="text-green-400 text-lg font-mono mb-2">{loadingPhases[currentPhase]?.text || "READY"}</p>
              <motion.div
            ...
            <motion.div
              ...
              style={{
                ...
                opacity: isGlitching ? Math.random() * 0.8 + 0.2 : 0.5,
              }}
              animate={{ opacity: isGlitching ? [0.2, 0.9, 0.3] : [0.5, 0.8, 0.5] }}
              transition={{ duration: isGlitching ? 0.1 : 2, repeat: Infinity }}
            />
          </div>
        </motion.div>
      )}
