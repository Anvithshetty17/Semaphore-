...
<title>${heading || 'Semaphore 2K24'}</title>
<style>
...
<body style="margin:0;padding:0;background:#050011;font-family:'Segoe UI',Arial,sans-serif;">
        ...
        <table role="presentation" width="640" cellspacing="0" cellpadding="0" style="max-width:640px;width:100%;background:rgba(5,0,25,0.72);backdrop-filter:blur(4px);border:1px solid #2a1455;border-radius:20px;box-shadow:0 0 25px rgba(138,43,226,0.55),0 0 60px rgba(0,255,255,0.15);overflow:hidden;">
          <tr>
            <td style="padding:32px 36px;color:#ffffff;">
              <div style="font-size:11px;letter-spacing:4px;text-transform:uppercase;color:#00eaff;font-weight:600;margin:0 0 14px;">Semaphore 2K24</div>
              ...
              <p style="margin:0;font-size:11px;line-height:1.5;color:#8b92b0;">If you didn't expect this email you can safely ignore it. © Semaphore 2K24</p>
            </td>
          </tr>
        </table>
      </td>
  ...
  async sendEmailVerificationMail(
    ...
    const subject: string = '🔐 Verify Your Email – Enter the Grid (Semaphore 2K24)';
    ...
    <p style='margin:0 0 16px;'>You are moments away from jacking into <strong>Semaphore 2K24</strong> at <strong>NMAMIT, Nitte</strong>. We just need to verify this address is really you.</p>
    ...
    const mailOptions = {
      ...
      to: toEmailId,
      subject,
      text: `Verify your email to finalize your Semaphore 2K24 registration: ${verificationLink}`,
  ...
  async sendPaymentAcceptedEmail(
    ...
    const subject = `✅ Registration Locked In – See You at Semaphore 2K24`;
    const inner = `<p>Hey <strong>${fullName}</strong>,</p>
    <p>Your payment has <span style='color:#00eaff;'>cleared the system</span>. Your slot is now <strong style='color:#ff70ff;'>officially confirmed</strong> for <strong>Semaphore 2K24</strong>.</p>
    <h3 style='margin:26px 0 12px;font-size:17px;color:#ff70ff;'>Event Coordinates</h3>
    <ul style='padding:0 0 0 18px;margin:0 0 18px;'>
      <li>📅 19 – 20 Nov 2024</li>
    ...
    <p style='margin:30px 0 0;'>Welcome aboard,<br><span style='color:#00eaff;'>Semaphore 2K24 Crew</span></p>`;
    const mailOptions = {
      from: this.configService.get<string>('GMAIL_USER'),
      to: toEmailId,
      subject,
      text: `Your Semaphore 2K24 registration is confirmed. Event: 19–20 Nov 2024 @ NMAMIT.`,
  ...
  async sendPaymentRejectedEmail(
    ...
    const subject = `⚠ Payment Issue – Action Needed (Semaphore 2K24)`;
    const inner = `<p>Hi <strong>${fullName}</strong>,</p>
    <p>Your payment attempt for <strong>Semaphore 2K24</strong> didn't pass validation.</p>
    ...
    const mailOptions = {
      ...
      to: toEmailId,
      subject,
      text: `Payment issue for Semaphore 2K24. Reason: ${remarks}. Please retry with correct details.`,
  ...
  async sendNextRoundSelectedEmail(
    ...
    const inner = `<p>Hey <strong>${fullName}</strong>,</p>
    <p>Your team just <span style='color:#00eaff;'>leveled up</span> to <strong>Round ${roundNo}</strong> of <strong>${eventName}</strong> at <strong>Semaphore 2K24</strong>.</p>
    ...
    const mailOptions = {
      ...
      to: toEmailId,
      subject,
      text: `Selected for Round ${roundNo} of ${eventName} at Semaphore 2K24. Keep going!`,
  ...
  async sendPasswordResetLinkEmail(
    ...
    <p>A password reset signal was received for your <strong>Semaphore 2K24</strong> account. If this wasn't you, disregard and your credentials stay intact.</p>
    ...
    const mailOptions = {
      from: this.configService.get<string>('GMAIL_USER'),
      to: toEmail,
      subject: 'Password Reset Link – Semaphore 2K24',
      text: `Reset your Semaphore 2K24 password: ${resetLink}`,
